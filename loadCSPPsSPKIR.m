...
if downloadTHREDDS
    for nsite=sites
        ...
        stream = 'spkir_j_cspp_instrument_recovered';
        tag = '.*SPKIR.*\.nc$';
...
% no spkir data nsite= 2 deployment= 27
...
if inspectQC
    ...
    for nsite = sites
        ...
        for i=1:1:max(ce.deployment)
            ...
            night_spkir_j_par_counts_output = ce.spkir_j_par_counts_output(dex);
            night_spkir_j_par_counts_output_qartod_results = ce.spkir_j_par_counts_output_qartod_results(dex);

            dex = find(ce.deployment==i & ce.day);
            depth = ce.depth(dex);
            profiler_datetime = ce.profiler_datetime(dex);
            spkir_j_par_counts_output = ce.spkir_j_par_counts_output(dex);
            spkir_j_par_counts_output_qartod_results = ce.spkir_j_par_counts_output_qartod_results(dex);

            if ~isempty(dex)
                subplot(211);
                scatter(profiler_datetime,depth,12,log10(spkir_j_par_counts_output),'filled')
                ...
                % end
                dex=find(spkir_j_par_counts_output_qartod_results>1);
                if ~isempty(dex)
                    plot(profiler_datetime(dex),spkir_j_par_counts_output_qartod_results(dex),'r.','MarkerSize',12);
                end
                title('CSPP log10(PAR) with red QARTOD');

                subplot(212);
                scatter(night_profiler_datetime,night_depth,12,log10(night_spkir_j_par_counts_output),'filled')
                ylabel('log10(PAR)'); hold on; set(gca,'ydir','rev'); 
                a=colorbar; %set(a,'limits',[-1 3.2]);
                dex=find(night_spkir_j_par_counts_output_qartod_results>1);
                if ~isempty(dex)
                    plot(night_profiler_datetime(dex),night_spkir_j_par_counts_output_qartod_results(dex),'r.','MarkerSize',12);
                end
